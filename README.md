...
- Extremely robust LLM fallbacks ensuring demo/uptime functionality regardless of API status
- Resume text persisted in PostgreSQL — survives server restarts on ephemeral platforms (Render, Heroku)
...
- **File Storage**: `uploads/` (resumes, proctoring snapshots, exports)
  - **Ephemeral on Render/Heroku**: Files in `uploads/` are wiped on every server restart
