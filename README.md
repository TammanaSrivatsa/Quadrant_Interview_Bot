...
- **Route Groups**:
  ...
  - ▶️ `routes/interview/runtime.py` — session start, answer submission, transcription, proctoring events
...
- 🤔 **Phase 2 (Question Generation)**:
  ...
  - *Logic*: Projects extracted from resume + weighted skill distribution (80% technical / project-based, 20% behavioral)

- 🎙️ **Phase 3 (Interview Runtime)**:
...
- ✅ **Resume Text Persistence**: Extracted text stored in PostgreSQL mapping for ephemeral filesystem robustness
...
- 🗂️ **Bulk Actions**: Multi-select candidates for bulk shortlisting/rejection
- 📅 **Interview Calendar**: Interactive modal for scheduling candidate interviews

### 🎤 Interview Workflow (Phase 3)
- 🛡️ **Indestructible AI Generation**: The LLM question generator and answer evaluator have extreme, unbreakable fallbacks to prevent flow crashes.
- ⚡ **Groq Whisper STT**: The backend seamlessly utilizes Groq API for instantaneous speech-to-text transcriptions.
...
- 🛡️ **Graceful CV Failure**: OpenCV proctoring errors are swallowed securely to prevent backend 500 crashes during demonstrations.
...
- ⏸️ **Pause on Warnings**: Optional enforcement (`PROCTOR_PAUSE_ENABLED` env var)
- ⏱️ **Periodic Snapshots**: Configurable interval for suspicious events only
...
> - **Groq key** (for transcription): Get from console.groq.com
> - **Gemini key** (for intelligence): Get from Google AI Studio
...
3. Result: Screening band (`strong_shortlist` / `review_shortlist` / `reject`) + explanations
